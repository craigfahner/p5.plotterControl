...
// function keyPressed(){
//   if(key==="c"){
//     drawString("Hello", 50,500, 0.5);
//   } else if(key==="e"){
//     plotter.circle(350,350,50,false);
//   } else if(key==="t"){
//     drawString(String(pos), 10+(pos+10),475, 0.5);
//     pos++;
//   } else if(key==="q"){
...
function mousePressed() {
  if (mouseX > 0 && mouseX < width && mouseY > 0 && mouseY < height) {
    if (key === "t") {
      drawString("Hello", mouseX, mouseY, 0.5);
    } else if (key === "c") {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (glyph && glyph.d) {
      Array.prototype.push.apply(
        commands,
        getPathCommandsForChar(glyph.d, offset),
      );
      offset += int(glyph.o) * 2;
    } else {
      // fallback spacing for missing glyphs, spaces, accents, etc.
      // console.warn("Unsupported character: " + s.charAt(i));
      offset += 10;
    }
  }
  ...
  for (let cmd of cmds) {
    switch (cmd.type) {
      ...
      case "L":
        ...
        plotter.line(x0, y0, x1, y1); //ak edit
        stroke(255); //ak edit
        //currCol=!currCol; //ak edit
        cx = cmd.x;
        cy = cmd.y;
